...
public abstract class CANDevice {
  ...
  private static final int THRIFTY_VENDOR_ID = 13;
  ...
  protected CANDevice(int canID, DeviceType type) {
    this.canID = canID;
    this.deviceHandle = CANAPIJNI.initializeCAN(THRIFTY_VENDOR_ID, canID, type.ordinal());
  }
  ...
  protected int reciveStatusFrame(int index, int startByte, int endByte) {
    ...
    CANAPIJNI.readCANPacketLatest(deviceHandle, APIClass.STATUS.get(index), dataBuffer);
