...
public class TalonFXPID extends GenericPIDController {
  ...
  private void sendControlRequest(double reference, double feedforward) {
    switch (controlType) {
      ...
      case DUTY_CYCLE:
        ((TalonFX) motor.getMotor())
            .setControl(new DutyCycleOut(reference).withEnableFOC(motor.isFOCEnabled()));
        break;
      case VOLTAGE:
        ((TalonFX) motor.getMotor())
            .setControl(new VoltageOut(reference).withEnableFOC(motor.isFOCEnabled()));
        break;
      default:
  ...
  /**
   * Sets the feedforward value of the PID controller using the kS constant. This is definitely not
   * right.
   *
  ...
  public void setF(double f) { // TODO: This definitely shouldn't just set kS
    cfg.refresh(configuration.Slot0);
    cfg.apply(configuration.Slot0.withKS(f));
  }
